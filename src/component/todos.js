...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      ...
      const dueDate = new Date(todo.due).toLocaleDateString();
      ...
      return (
        <Grid key={todo.id}>
          <Card style={{background:color, marginTop:10}} data-testid={todo.content}>
            ...
            <ListItemButton component="a" href="#simple-list">
              ...
              <ListItemText primary={todo.content} secondary={todo.due.toLocaleString("en-US")}/>
            </ListItemButton>
          </Card>
        </Grid>
      );
